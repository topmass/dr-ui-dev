...
export default function AdminDashboard() {
  ...
  const handleDecision = (decision: string) => {
    if (decision === "AI was correct") {
      // Direct feedback for AI correct case
      console.log(`Decision made: ${decision}`)
      alert(`Feedback "${decision}" sent to CSR`)
      setSelectedConversation(null)
    } else {
      // Open dialog for other cases
      setFeedbackType(decision)
      setShowFeedbackDialog(true)
    }
  }

  const handleSubmitFeedback = () => {
    console.log(`Decision made: ${feedbackType}`, { feedbackNote })
    setShowFeedbackDialog(false)
    setSelectedConversation(null)
    setFeedbackNote("")
  }

  const handleFeedbackNoteChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900 relative">
                            ...
                            {entry.speaker === "AI" ? (
                              <div className="bg-gray-100 dark:bg-gray-800 p-2 rounded">
                                ...
                                <div className="mt-2 flex items-center">
                                  ...
                                  <Badge variant={entry.csrResponse === "Yes" ? "secondary" : "destructive"}>
                                    {entry.csrResponse}
                                  </Badge>
                                </div>
                              </div>
                  ...
                  <CardFooter className="flex flex-col space-y-4">
                    ...
                    <div className="flex justify-between w-full">
                      ...
                      <div className="space-x-2">
                        <Button variant="secondary" onClick={() => handleDecision("CSR was correct")}>
                          CSR was correct
                        </Button>
      ...
      <Dialog open={showFeedbackDialog} onOpenChange={setShowFeedbackDialog}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>
              {feedbackType === "CSR was correct" ? "Confirm CSR Feedback" : "Confirm Alternative Solution"}
            </DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="text-sm text-neutral-600 dark:text-neutral-400">
              {feedbackType === "CSR was correct" ? (
                "Anonymous information about this interaction will be sent to Dataripple to improve the model. This scenario will be added to examples training the model's behavior."
              ) : (
                "This feedback will help us improve both AI and CSR training. Anonymous information will be used to refine our models and procedures."
              )}
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <label htmlFor="feedback" className="text-sm font-medium col-span-4">
                {feedbackType === "CSR was correct" 
                  ? "Additional notes for model improvement:"
                  : "Describe the correct approach:"}
              </label>
              <Textarea
                id="feedback"
                value={feedbackNote}
                onChange={handleFeedbackNoteChange}
